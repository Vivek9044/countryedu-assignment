...
MODEL_PATH = os.path.join(MODELS_DIR, 'escalation_model.joblib')

def train_and_evaluate_escalation_model():
    ...
    print(f"Loading cleaned dataset for escalation prediction model...")
    df = pd.read_csv(cleaned_path)
    
    # Target variable: 1 if escalation_count > 0, else 0
    df['is_escalated'] = (df['escalation_count'] > 0).astype(int)
    
    # Check class distribution
    class_counts = df['is_escalated'].value_counts()
    print(f"Escalation class distribution:\n{class_counts}")
    imbalance_ratio = class_counts[0] / class_counts[1]
    print(f"Imbalance ratio (Non-Escalated / Escalated): {imbalance_ratio:.2f}")
    ...
    print(f"Saved best escalation model to {MODEL_PATH}")
    
    # Save comparison report
    report_path = 'reports/escalation_model_report.txt'
    with open(report_path, 'w') as f:
        f.write("--- ESCALATION MODEL COMPARISON ---\n")
...
def predict_single_ticket(ticket_dict):
    """
    Predict the escalation probability and binary flag for a single ticket.
    ...
    return {
        ...
        'escalation_prediction': int(pred),
        'latency_seconds': latency
    }

if __name__ == '__main__':
    train_and_evaluate_escalation_model()
